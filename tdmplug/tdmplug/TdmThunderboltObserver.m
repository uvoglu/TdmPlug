...
#import "TdmThunderboltObserver.h"
#import "TdmSystemProfiler.h"

@interface TdmThunderboltObserver () <NetworkInterfaceNotifierDelegate>
...
@implementation TdmThunderboltObserver
...
- (void)networkInterfaceNotifierDidDetectChanage {
    ...
    NSLog(@"Network interface change detected");
    [self checkForThunderboltConnection];
}

- (void)startObserving {
    NSLog(@"Start observing thunderbolt connections");
...
- (void)stopObserving {
    ...
    NSLog(@"Stop observing thunderbolt connections");
...
- (void)updateSystemProfilerInformation {
    self.systemProfilerInformation = [TdmSystemProfiler dataTypes:@[@"SPDisplaysDataType", @"SPThunderboltDataType"]];
}
