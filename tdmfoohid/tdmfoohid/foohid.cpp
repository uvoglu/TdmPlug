...
OSDefineMetaClassAndStructors(com_uvoglu_tdmfoohid, IOService);

bool com_uvoglu_tdmfoohid::start(IOService *provider) {
    LogD("Executing 'com_uvoglu_tdmfoohid::start()'.");
    
    bool ret = super::start(provider);
    if (ret) {
        LogD("Calling 'com_uvoglu_tdmfoohid:registerService()'.");
        registerService();
    }
...
void com_uvoglu_tdmfoohid::stop(IOService *provider) {
    LogD("Executing 'com_uvoglu_tdmfoohid:stop()'.");
    ...
    if (iter) {
        ...
        while ((key = (OSString *)iter->getNextObject())) {
            com_uvoglu_tdmfoohid_device *device = (com_uvoglu_tdmfoohid_device *)m_hid_devices->getObject(key);
...
bool com_uvoglu_tdmfoohid::init(OSDictionary *dictionary) {
    LogD("Executing 'com_uvoglu_tdmfoohid:init()'.");
...
void com_uvoglu_tdmfoohid::free() {
    LogD("Executing 'com_uvoglu_tdmfoohid:free()'.");
...
bool com_uvoglu_tdmfoohid::methodCreate(char *name, UInt8 name_len,
    ...
    com_uvoglu_tdmfoohid_device *device = nullptr;
    ...
    device = (com_uvoglu_tdmfoohid_device *)m_hid_devices->getObject(key);
    if (device) return false;
    
    device = OSTypeAlloc(com_uvoglu_tdmfoohid_device);
    if (!device) goto fail;
...
bool com_uvoglu_tdmfoohid::methodDestroy(char *name, UInt8 name_len) {
    OSString *key = nullptr;
    com_uvoglu_tdmfoohid_device *device = nullptr;
...
bool com_uvoglu_tdmfoohid::methodSend(char *name, UInt8 name_len,
                                 unsigned char *report_descriptor,
                                 UInt16 report_descriptor_len) {
    com_uvoglu_tdmfoohid_device *device = nullptr;
    OSString *key = nullptr;
...
bool com_uvoglu_tdmfoohid::methodList(char *buf, UInt16 buf_len,
...
bool com_uvoglu_tdmfoohid::methodSubscribe(char *name, UInt8 name_len, IOService *userClient) {
    OSString *key = nullptr;
    com_uvoglu_tdmfoohid_device *device = nullptr;
